...
ACM's contribution is not inventing these patterns but recognizing that **agent memory is an authorization domain** where they apply. Prior agent memory models (Section 5.1â€“5.3) treat memory as a capability resource. ACM treats memory as an authorization substrate â€” the place where the mandate lives, where the agent's interpretation is recorded, and where independent evidence of what happened accumulates.
